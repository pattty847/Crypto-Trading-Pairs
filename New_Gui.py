...
class Graphics():
    def __init__(self, api):
        ...
        if not isfile("settings.json"):
            ...
            setting_init = {"last_symbol": "BTC/USDT", "last_timeframe": "1h", "last_since": month_ago}
    ...
    def trade(self): 
            ...
            with dpg.child_window(autosize_x=True, height=(self.viewport_height/2), pos=[self.viewport_width - self.side_panel_width, 7]):
                with dpg.collapsing_header(label="Trade"):
                    ...
                    with dpg.tree_node(label="Symbol"):
                        with dpg.group(horizontal=True):
                            dpg.add_listbox(self.api.symbols, tag='symbols-listbox', default_value=self.settings['last_symbol'], width=-1, callback=self.set_symbol, num_items=50)


                    with dpg.group(horizontal=True):
                        with dpg.group(horizontal=False):
                            dpg.add_text("Timeframe")
                            dpg.add_listbox(self.api.timeframes, tag='timeframe-listbox', default_value=self.settings['last_timeframe'], width=75, callback=self.set_timeframe)
                        with dpg.group(horizontal=False):
                            dpg.add_text("From")
                ...
                (dates, opens, closes, lows, highs) = self.do.candles_to_list(candles)
                self.update_chart_series(None, None, None, dates, opens, closes, lows, highs, self.settings['last_symbol'], self.settings['last_timeframe'], False)
        ...
        with dpg.child_window(autosize_x=True, height=-1, pos=[self.viewport_width - self.side_panel_width, self.viewport_height/2+5]):
            with dpg.collapsing_header(label="Indicators"):
                with dpg.group(horizontal=True):
                    dpg.add_button(label="SMA")
                    dpg.add_button(label="EMA")
                    dpg.add_button(label="RSI")
